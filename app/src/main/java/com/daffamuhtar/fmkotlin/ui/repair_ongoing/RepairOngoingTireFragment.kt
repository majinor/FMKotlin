...
import com.daffamuhtar.fmkotlin.databinding.FragmentRepairOngoingTireBinding
...
class RepairOngoingTireFragment : Fragment() {

    private lateinit var repairOngoingTireViewModel: RepairOngoingTireViewModel

    private lateinit var binding: FragmentRepairOngoingTireBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentRepairOngoingTireBinding.inflate(inflater)
        return binding.root
    }
    ...
    private fun callData() {
        repairOngoingTireViewModel.getRepairTire(context, ConstantaApp.BASE_URL_V2_0, "MEC-MBA-99")

    }

    private fun initViewModel() {
        repairOngoingTireViewModel.repairList.observe(requireActivity()) {
            if (it.isNotEmpty()) {
                setReportResults(it)
            } else {
        ...
        repairOngoingTireViewModel.isLoadingGetRepair.observe(requireActivity()) {
            loading(it)
        }
    ...
    private fun declare() {
        ...
        repairOngoingTireViewModel = ViewModelProvider(this)[RepairOngoingTireViewModel::class.java]
    }
